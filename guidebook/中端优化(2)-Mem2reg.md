...
这一章我们将介绍如何将一个非SSA的形式转化到含有phi函数的SSA形式。首先看下面的一个例子，让大家看看mem2reg的神效（大家也可以做完后用这个例子做测试，生成内容无需完全一致，仅作参考）：
...
是不是代码精简了很多？回忆上次的内容，我们在介绍SSA时提及的两个主要的方法，一个是改名，一个是引入phi函数。我们这里也将主要按照这样的方法对SSA形式进行构建。SSA构建的标准算法包含两个过程：$\phi\ Insertion$ 和$Variable\ Renaming$

## phi Insertion

在这一个步骤里，我们会把构建SSA所需要插入的phi函数都一口气先插到里面。想到这的时候，我们可以先自己试着想一想，什么时候我们需要插入phi？（忘了phi指令的可以回顾上一章）

答案是当我们遇到不同基本块汇集（converge）到同一个基本块的时候。看下面的例子：

![3ddc02aaac8cc8222c938b2fae33b2f.png](https://s2.loli.net/2023/04/12/vNZbMFSluXUjTOn.png)

我们将这种展示程序流程的图叫做控制流程图（**C**ontrol **F**low **G**raph），简记为**CFG**。上图中，B, C基本块汇聚到D，这导致我们无法确定y此时的取值来自于B还是C，只好插入phi函数实现SSA。一个直观的想法就是所有这种汇聚的基本块中都需要插入phi函数。我们用更加专业的语言来定义一下汇聚这个概念：
...
在明确Join set的概念后，同学们可以先自己试着想一想，对于一个变量v，应该在哪些基本块插入和它相关的phi指令？（可以考虑上面的例子，如变量y我们分析的过程）



在上面的例子中，变量y在B,C中被定义(def)，然后我们发现 $J(\{B,C\})=\{D\}$ ,因此在D中要构建phi指令。那么抽象出来就是对于一个变量v，我们先找到它的定义块集合 $Def_v$ ，简记为 $D_v$ ，然后再计算 $J(D_v)$ ，得到的即为我们应该插入phi指令的基本块集合。有些比较严谨的同学可能会想到，phi指令本身，不也是对变量v的定义嘛？确实是这样，因此我们其实再构建phi函数的时候，又引入了新的定义，所以实际上应该是 $J(D_v∪J(D_v))$ 但由于 $J(D_v∪J(D_v))=J(D_v)$ （这一部分不做详细证明，感兴趣的同学可以自己举例想一想），所以我们最终需要插入phi指令的基本块集合**还是 $J(D_v)$** 。



似乎问题到这里已经解决了，我们只需要遍历每个变量，计算出 $J(D_v)$ ，然后插入phi指令就好了，但是稍加分析or尝试就可以发现，求 $J(D_v)$ 的过程非常复杂。但是一些非常聪明的计算机科学家结合了图论中的一些概念，将问题转化的更好求解。下面介绍一下相关概念：
...
大家在熟悉上述概念后，接下来就要介绍最核心的概念啦：**支配边界**
...
DF^{+}(S)=DF_{i->\infty}
$$
说了这么多，大家还记得我们想要求的 $J(Def)$ 嘛？事实上（该公式感兴趣的同学可以自行证明）:
$$
DF^{+}(S)=J(S\ \cup\ entry)
$$
...
在支配树上进行 DFS，DFS 的过程中，计算并更新每个变量 �*v* 当前的定义 �.reachingDef*v*.reachingDef，并创建新的变量。算法如下：
